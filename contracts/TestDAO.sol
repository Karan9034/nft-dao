...
interface TestMarketplaceInterface {
...
contract TestDAO is Ownable {
    ...
    TestMarketplaceInterface marketplace;
    TestNFTInterface nft;

    constructor(address _marketplace, address _nft) payable {
        marketplace = TestMarketplaceInterface(_marketplace);
        nft = TestNFTInterface(_nft);
    }
    ...
    function createProposal(uint256 _nftTokenId) external nftHolderOnly returns (uint256) {
        require(marketplace.available(_nftTokenId), "NFT_NOT_FOR_SALE");
